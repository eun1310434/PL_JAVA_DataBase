...
public class RandomClient extends Thread {
	...
	public RandomClient(DBConnectionPool _pool,int pos) {
		this.pool = _pool;
		this.pos = pos;
	}
	
	//서버에 접속해서 일정 업무를 처리하는 Client를 가정한다.
	public void run() {
		//Client 접속
		try {Thread.sleep((int) (Math.random() * 10000));} 
		catch (InterruptedException e) {e.printStackTrace();}
		Connection connection = pool.getConnection();
		System.out.println("connection[" + pos + "] - Create");
		...
		System.out.println("connection[" + pos + "] - Release");
	}
}