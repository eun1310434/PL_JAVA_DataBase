...
public class DBConnectionFactoryRun {
	public static void main(String[] ar) {
		
		DBGetData getData = new DBGetData();
		
		String[] user_sn = {"AAA","BBB","CCC", "DDD", "EEE"};
		for(int i = 0 ; i< user_sn.length ; i++){
			
			//접속자가 생길때 마다 connection을 각각 생성한다.
			Connection connection = DBConnectionFactory.getInstance();	
			
			//결과값을 갖고온다.
			getData.getUserInfo(connection, user_sn[i]);
		...
		DBConnectionFactory.getConnectionCount();
